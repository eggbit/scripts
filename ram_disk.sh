...
ramdisk_mount() {
    ...
    if hash zpool 2> /dev/null; then
        zpool create -f -o ashift=10 -O casesensitivity=insensitive -O normalization=formD -O atime=off -O compression=lz4 -O checksum=off -O sync=disabled -O primarycache=none -O secondarycache=none $1 $RAMDISK_MOUNT
    else
